...
nodetoo.boostrap = function(app) {
  ...
  // configure i18n
  i18nConfig(app);
  readInjections(nodetoo.root + '/config/injections');
...
function readInjections(dir) {
  fs.readdirSync(dir).forEach(function (item) {
    var stats = fs.statSync(dir + '/' + item);
    if (stats.isFile()) {
      var middleware = require(dir + '/' + item);
      nodetoo.inject[path.basename(item, '.js')] = middleware;
    }
  })
}
